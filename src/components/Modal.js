...
const Modal = ({ modal, data, setModal, setData }) => {
  ...
  const [urlTrailler, setUrlTrailer] = useState(null);
  ...
  async function playTrailler(name) {
    try {
      const response = await movieTrailer(name);
      setUrlTrailer(response);
      if (!response) {
        console.log('false');
        setNoTrailer(true);
      }
    ...
    <div>
      {results && (
        <div
          ...
          onClick={handleClick}
        >
          {urlTrailler && <ReactPlayer playing={true} url={urlTrailler} />}
          {!urlTrailler && (
            <div className={styles.modalConteiner}>
              ...
              <div className={styles.modalContent}>
                  ...
                  <div>
                    ...
                    >
                      ...
                      {moreDetails.release_date && (
                        <p>
                          {new Date(moreDetails.release_date).getFullYear()}
                        </p>
                      )}
                      {moreDetails.runtime && (
                ...
                <button
                  onClick={() =>
                    playTrailler(
                      results[indexId].name || results[indexId].title,
                    )
                  }
                >
                  {noTrailer ? 'Não disponível' : 'Assistir'}
                </button>
              </div>
            </div>
          )}
